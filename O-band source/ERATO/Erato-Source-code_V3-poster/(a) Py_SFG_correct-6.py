...
def fit_weighted_gauss(x, y, s):
    s = np.asarray(s, dtype=float).copy()  # writable copy for pandas/NumPy views
...
def draw_fig8_with_theory_log(spec_full_4, mu_fourstep, FWHM_fourstep, A_fourstep, xfull,
    ...
    lg, = ax.semilogy(xd, y_g_norm, color=COL_GAUSS, lw=GAUSS_LW, label="Fitted Gaussian")
    ...
    ax.semilogy(xs, y_th_lin, color="#b22222", lw=1.8, label="Optimized PMF (idler theory)")
    ...
    with open("poster_a.pdf", "wb") as pdf_file:
...
draw_fig8_with_theory_log(
    ...
    mu_C4, FWHM_C4, A_C4,
    xfull,
    out_png="fig8_full_4step_log_withTheory.pdf"
)
...
# =========================================================
#  Fig17: full 4-step, with *shifted* & *original* PMF (linear)
#  在 fig16 之后追加这一段
# =========================================================

def theory_curve_unshifted_linear(x_range, step_nm=0.01):
    """
    不做中心波长 shift 的理论 PMF 曲线（idler 方向，线性标度）。
    x_range: (xmin, xmax) in nm
    """
    li = np.arange(x_range[0], x_range[1] + 1e-9, step_nm)
    y_lin = (phi2_lambda(LAM_S0, li)**2)
    y_lin /= np.nanmax(y_lin)  # 归一化到 1
    return li, y_lin


def draw_fig17_with_shift_and_unshift_linear(
    spec_full_4,
    mu_fourstep, FWHM_fourstep, A_fourstep,
    x_range,
    out_png="fig17_full_4step_withTheoryshift&Theory_linear.png"
):
    """
    叠加：数据 + Gaussian + sinc^2 + PMF(shifted) + PMF(original)  —— 线性 y 轴
    x_range 设定横轴范围，例如 (1520, xfull[1])
    """
    x_data = spec_full_4["wl"].to_numpy()
    y_data = spec_full_4["y"].to_numpy()
    s_data = spec_full_4["yerr"].to_numpy()
    ...
    # Gaussian + sinc^2（和 fig6/fig16 一致）
    xd = np.linspace(x_range[0], x_range[1], 1400)
    sigma = FWHM_fourstep / (2 * np.sqrt(2 * np.log(2)))
    y_g = gauss(xd, A_fourstep, mu_fourstep, sigma)
    y_s = sinc2_curve(xd, mu_fourstep, FWHM_fourstep, A_fourstep)

    # 理论曲线：shifted（对齐 mu_fourstep）和 original（不 shift）
    xs_shift, y_th_shift_norm = theory_curve_shifted_linear(
        mu_fourstep, x_range, step_nm=0.005
    )
    xs_org, y_th_org_norm = theory_curve_unshifted_linear(
        x_range, step_nm=0.005
    )

    # 计算两条理论曲线的 FWHM（仅打印到终端供你参考）
    fwhm_shift = calculate_numeric_fwhm(xs_shift, y_th_shift_norm)
    fwhm_org   = calculate_numeric_fwhm(xs_org,   y_th_org_norm)
    print("-" * 60)
    print("Fig.17 Theory Curves:")
    print(f"  shifted  theory FWHM ≈ {fwhm_shift:.4f} nm")
    print(f"  original theory FWHM ≈ {fwhm_org:.4f} nm")
    print("-" * 60)
    ...
    if len(y) > 0:
        ...
        y_peak = 1.0
    y_th_shift = y_th_shift_norm * y_peak
    y_th_org   = y_th_org_norm   * y_peak
    ...
    # Gaussian
    ax.plot(xd, y_g, color=COL_GAUSS, lw=GAUSS_LW, label="Fitted Gaussian")

    # sinc^2（长划线虚线）
    line_s, = ax.plot(xd, y_s, color=COL_SINC2, lw=SINC_LW, label="sinc²")
    try:
        line_s.set_linestyle((0, (6, 6)))
    except TypeError:
        line_s.set_dashes([6, 6])

    # shifted 理论：实线
    ax.plot(
        xs_shift, y_th_shift,
        color=COL_PMF, lw=1.8,
        label="Optimized PMF (shifted theory)"
    )
    # original 理论：同色虚线
    ax.plot(
        xs_org, y_th_org,
        color=COL_PMF, lw=1.4, linestyle="--",
        label="Optimized PMF (original theory)"
    )
    ...
    title1 = "Four-step BG-removed Normalized SFG vs Wavelength (C+L)"
    title2 = "Shifted & original CPKTP PMF (linear)"
    ax.set_title(title1 + "\n" + title2)
    ...
    # Additional poster export: same canvas ratio/font sizes as poster_a.
    fig.set_size_inches(8.8, 6.5)
    ax.title.set_visible(False)
    ax.set_xlabel("Wavelength (nm)", fontsize=30)
    ax.set_ylabel("Normalized SFG (a.u.)", fontsize=30)
    ...
    fig.tight_layout(pad=0.7)
    fig.savefig("poster_fig17.png", dpi=300)
    with open("poster_fig17.pdf", "wb") as pdf_file:
        fig.savefig(pdf_file, format="pdf")
    plt.close(fig)
    print("Saved:", out_png)


# ===== 调用：保持 fig16 不变，再额外画 fig17 =====
# 画横轴从 ~1520 nm 开始，右端沿用 xfull[1]
x17_range = (1520.0, xfull[1])

draw_fig17_with_shift_and_unshift_linear(
    spec_full_4,
    mu_C4, FWHM_C4, A_C4,
    x_range=x17_range,
    out_png="fig17_full_4step_withTheoryshift&Theory_linear.pdf"
)
