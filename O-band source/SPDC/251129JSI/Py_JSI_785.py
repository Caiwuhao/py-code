...
def robust_solve_idler(ls_nm):
    ...
    if len(sign_changes_neg) > 0:
        ...
        try:
            return brentq(target_func_neg, scan_range[idx], scan_range[idx+1])
        except:
            pass
...
if wl_exp is None:
    ...
    wl_exp = np.linspace(1530, 1580, 100)
    int_exp = np.exp(-(wl_exp - 1556)**2 / (2 * 1.0**2))

# ========= 3. 原始 705 nm pump 的实验峰值 & Shift =========
peak_idx_orig = np.argmax(int_exp)
peak_wl_exp_orig = wl_exp[peak_idx_orig]
fwhm_exp_orig = calculate_fwhm(wl_exp, int_exp)

print(f"Original Experimental Peak (705 nm pump): {peak_wl_exp_orig:.3f} nm")
print(f"Original Experimental FWHM: {fwhm_exp_orig:.3f} nm")
...
# 与原程序一致：理论 idler 与实验峰值之间的大约 20 nm shift
SHIFT_VAL = peak_wl_exp_orig - theo_idler_ref_705
print(f"Applied Shift (theory -> exp): {SHIFT_VAL:.3f} nm")

# ========= 4. 计算 785 nm pump 的中心 Signal/Idler =========
PUMP_TARGET_NM = 785.0

def solve_signal_for_pump(lambda_pump_nm, s_min=1200.0, s_max=2000.0, n_scan=400):
    """
    给定泵浦波长 (nm)，在 [s_min, s_max] 范围内搜索满足
    Δk(ls, li(ls)) + 2π/Λ = 0 的 signal 波长，
    其中 li 由能量守恒 1/λ_p = 1/λ_s + 1/λ_i 给出。
    """
    ls_scan = np.linspace(s_min, s_max, n_scan)
    dk_vals = []
    def delta_k_QPM(ls_nm):
        li_nm = lambda_pump_nm * ls_nm / (ls_nm - lambda_pump_nm)
        return get_k_material(ls_nm, li_nm) + 2*np.pi/LAMBDA_NM

    for ls in ls_scan:
        if ls <= lambda_pump_nm:  # 排除非物理解
            dk_vals.append(np.nan)
            continue
        dk_vals.append(delta_k_QPM(ls))
    dk_vals = np.array(dk_vals)

    # 搜索符号变号区间
    sign_changes = np.where(np.diff(np.sign(dk_vals)))[0]
    if len(sign_changes) > 0:
        idx = sign_changes[0]
        a, b = ls_scan[idx], ls_scan[idx+1]
        try:
            ls_root = brentq(delta_k_QPM, a, b)
        except:
            ls_root = 0.5*(a+b)
    else:
        # 没有严格变号，用绝对值最小点
        idx = np.nanargmin(np.abs(dk_vals))
        ls_root = ls_scan[idx]

    li_root = lambda_pump_nm * ls_root / (ls_root - lambda_pump_nm)
    return ls_root, li_root

signal_center_785, idler_center_th_785 = solve_signal_for_pump(PUMP_TARGET_NM)
idler_center_exp_785 = idler_center_th_785 + SHIFT_VAL

print(f"Pump = {PUMP_TARGET_NM:.1f} nm -> Center Signal(th) = {signal_center_785:.3f} nm, Idler(th) = {idler_center_th_785:.3f} nm")
print(f"Estimated Experimental Idler Center (with shift) = {idler_center_exp_785:.3f} nm")

# ========= 5. 根据 785 nm pump 重新定义实验 1D 分布与网格 =========
num_points = len(wl_exp)

# 旧波长范围相对于旧中心的左右宽度
left_width = peak_wl_exp_orig - wl_exp.min()
right_width = wl_exp.max() - peak_wl_exp_orig

li_min_new = idler_center_exp_785 - left_width
li_max_new = idler_center_exp_785 + right_width
li_uniform = np.linspace(li_min_new, li_max_new, num_points)

# 把原始 1D 分布平移到新的中心 (保持形状不变，GVM 角度不变的近似)
delta_center = idler_center_exp_785 - peak_wl_exp_orig
f_int = interp1d(wl_exp, int_exp, kind='linear', fill_value=0.0, bounds_error=False)
int_exp_uniform = f_int(li_uniform - delta_center)

# 更新“实验中心”和 FWHM 为 785 nm pump 的值，后续代码只用这组
peak_idx = np.argmax(int_exp_uniform)
peak_wl_exp = li_uniform[peak_idx]
fwhm_exp = calculate_fwhm(li_uniform, int_exp_uniform)

print(f"\n=== 785 nm pump (re-centered) ===")
print(f"Experimental Peak (shifted): {peak_wl_exp:.3f} nm")
print(f"Experimental FWHM (shifted): {fwhm_exp:.3f} nm")

# Signal 轴窗口保持与原程序相同宽度（约 70 nm），以新中心 signal_center_785 为中心
SIGNAL_WINDOW_WIDTH = 70.0
ls_vec = np.linspace(signal_center_785 - SIGNAL_WINDOW_WIDTH/2.0,
                     signal_center_785 + SIGNAL_WINDOW_WIDTH/2.0,
                     600)

LS, LI = np.meshgrid(ls_vec, li_uniform)

# ================= 6. 理论 PMF (785 nm pump 区域) =================
print(">>> Computing Theoretical PMF (Shifted, 785 nm pump region)...")
# 仍然用 SHIFT_VAL 把显示坐标 (exp) 映射回理论坐标
...
center_col_idx = np.argmin(np.abs(ls_vec - signal_center_785))
theo_profile_int = PMF_Theo_Amp[:, center_col_idx]**2 
fwhm_theo = calculate_fwhm(li_uniform, theo_profile_int)
print(f"Theoretical PMF FWHM (@ {signal_center_785:.1f} nm signal): {fwhm_theo:.3f} nm")

# ================= 7. 实验 PMF 重构 (785 nm pump 区域) =================
print(">>> Reconstructing Experimental PMF (Cyclic Rolling, 785 nm pump region)...")
...
for i, s_val in enumerate(ls_vec):
    target_wl = ridge_curve_shifted[i]   # 理论 + shift 之后的 idler 波长 (显示坐标)
    delta_wl = target_wl - peak_wl_exp   # 相对实验中心的偏移 (785 nm pump)
    if np.isnan(delta_wl):
        shift_idx = 0
    else:
        shift_idx = int(np.round(delta_wl / step_nm))
...
# ================= 8. 纯度寻优 (与原程序相同) =================
print(">>> Optimizing Pump Bandwidth (same style as original)...")
...
print(f"\n=== Results for 785 nm pump region ===")
...
def create_custom_blend_cmap():
    # 按照原程序重现 Mathematica 的 Hue Blend 配色
    N_half = 256
    # 1. 蓝色到橙色
