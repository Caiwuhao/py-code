...
def calculate_purity(pmf_amp_matrix, pump_fwhm_nm, ls_grid, li_grid):
    ...
    # 2.355 是 FWHM -> Sigma (Intensity)
    # 如果要转为 Amplitude Sigma, 应除以 1.665 (即 2*sqrt(ln2))
    # 这里保持您的原参数，您知道区别即可
...
# 1. PMF Theo
im1 = axes[0,0].imshow(PMF_Theo_Amp**2 + 1e-15, extent=extent, origin='lower', aspect='auto', cmap='inferno', norm=norm_cfg)
...
# 2. PMF Exp
im2 = axes[0,1].imshow(PMF_Recon_Amp**2 + 1e-15, extent=extent, origin='lower', aspect='auto', cmap='inferno', norm=norm_cfg)
...
I_JSA_Theo_Norm = I_JSA_Theo / np.max(I_JSA_Theo) # 峰值归一化

im3 = axes[1,0].imshow(I_JSA_Theo_Norm + 1e-15, extent=extent, origin='lower', aspect='auto', cmap='viridis', norm=norm_cfg)
...
I_JSA_Exp_Norm = I_JSA_Exp / np.max(I_JSA_Exp) # 峰值归一化

im4 = axes[1,1].imshow(I_JSA_Exp_Norm + 1e-15, extent=extent, origin='lower', aspect='auto', cmap='viridis', norm=norm_cfg)
