...
# Output files
OUT_PNG = "signal_projection_corrected_3step_fixed_projection_only.png"
...
ax.plot(
    ...
    f2,
    lw=1.8,
    label=rf"$|f(\lambda_s,\lambda_i={fixed_idler_display:.2f}\,\mathrm{{nm}})|^2$"
)

# FWHM arrow at half maximum
half_max = 0.5
imax = int(np.nanargmax(f2))
left = np.where(f2[:imax] < half_max)[0]
right = np.where(f2[imax:] < half_max)[0]

if len(left) > 0 and len(right) > 0:
    il = left[-1]
    ir = imax + right[0]
    x_left = signal[il] + (half_max - f2[il]) * (signal[il+1] - signal[il]) / (f2[il+1] - f2[il])
    x_right = signal[ir-1] + (half_max - f2[ir-1]) * (signal[ir] - signal[ir-1]) / (f2[ir] - f2[ir-1])

    ax.annotate(
        "",
        xy=(x_right, half_max),
        xytext=(x_left, half_max),
...
ax.set_title(
    "JSI projection onto signal\n"
    rf"fixed idler={fixed_idler_display:.3f} nm, pump center={pump_center_shifted:.3f} nm, "
    rf"FWHM={fwhm_f2:.3f} nm"
)
ax.grid(alpha=0.3)
...
# ---- JSI-only plot: square, no colorbar ----
# Mathematica-like hue path: low blue -> cyan -> green -> yellow -> red high.
...
# Keep the plotted box square. No colorbar, so the whole figure stays square-like.
ax.set_box_aspect(1)

fig.tight_layout()
fig.savefig(OUT_JSI_PNG, dpi=300)
plt.close(fig)
