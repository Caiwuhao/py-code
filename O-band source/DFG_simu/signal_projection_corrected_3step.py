...
# Output files
OUT_PNG = "signal_projection_corrected_3step_minfix_pumpFWHM.png"
OUT_CSV = "signal_projection_corrected_3step_minfix_pumpFWHM.csv"
OUT_JSI_PNG = "signal_projection_corrected_3step_minfix_pumpFWHM_JSI.png"
...
def phi_amp_from_k(k):
    ...
    k_arr = np.atleast_1d(k).astype(complex)
    j = np.arange(A_SIGN.size, dtype=float)
    out = np.empty_like(k_arr, dtype=complex)
    for t, kv in enumerate(k_arr):
        if abs(kv) < 1e-15:
            # limit: integral over each domain tends to d
            out[t] = np.sum(A_SIGN) * D_HALF_NM
        else:
            s = np.exp(1j * kv * D_HALF_NM * j) @ A_SIGN
            out[t] = 1j * s * (np.exp(-1j * kv * D_HALF_NM) - 1.0) / kv
    return np.abs(out).reshape(np.shape(k))
...
JSI_STEP_NM = 0.05   # if too slow, change to 0.1
