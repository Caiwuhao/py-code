...
# ——晶体与畴序列（与你给的完全一致；仅取前 265 个半畴）——
# ===== CPKTP theory (for Fig.8 overlay) =====
import numpy as np
...
# --- Sellmeier (Kato & Takaoka; λ in μm) ---
...
def draw_fig8_with_theory(spec_full_4, mu_fourstep, FWHM_fourstep, A_fourstep, xfull,
    ...
    # —— 底图：与 Fig.7 完全相同（数据均值转 dB，基准=数据峰值）——
    ...
    # —— 红线：理论 PMF（idler），横轴右移到 μ；红线自归一 0 dB —— 
...
# 8) Fig.8 —— 在 Fig.7 的基础上叠加红色理论曲线（正确做法）
draw_fig8_with_theory(
    spec_full_4,          # C+L 四步均值
    mu_C4, FWHM_C4, A_C4, # C-band 四步的加权高斯参数
    xfull,
    out_png="fig8_full_4step_dB_withTheory.png"
)
