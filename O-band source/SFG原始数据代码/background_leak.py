...
# ========= 读取数据（仅取第1列和第5列） =========
def read_sfg_file_col1_5(path):
    """读取含多列的 SFG 文件，仅保留第1列(wl)与第5列(sfg)"""
    ...
    return df.rename(columns={"col1": "wl", "col5": "sfg"})[["wl", "sfg"]]
...
if __name__ == "__main__":
    FILE = "SFG20251014101522.txt"
    df = read_sfg_file_col1_5(FILE)
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig("raw_SFG_from_col1_5.png", dpi=300)
    plt.close(fig)
    print("Saved: raw_SFG_from_col1_5.png")
