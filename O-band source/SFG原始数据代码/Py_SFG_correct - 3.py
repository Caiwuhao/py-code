...
def tight_ylim_from_y(y, pad_frac=0.04):
    ...
    if not np.isfinite(span) or span == 0:
        # 退化情形：给一个很小的上下边距
        delta = max(1e-12, abs(y_max) * pad_frac if np.isfinite(y_max) else 1e-3)
...
def draw_bg_mean(spec, out_png, title, label="mean (drop 50)"):
    ...
    ax.set_ylim(*tight_ylim_from_y(y))
...
# ========= Fig13: (BG01 + BG10 − BG00) raw mean vs Wavelength =========
def drop50_combo_bg(df):
    ...
    for wl, g in df.groupby("wl"):
        ...
        out.append((wl, float(np.mean(combo))))
    return pd.DataFrame(out, columns=["wl","y"]).sort_values("wl")

# 生成 C/L 两段并合并
specC_combo = drop50_combo_bg(dfC)
specL_combo = drop50_combo_bg(dfL)
spec_combo  = pd.concat([specC_combo, specL_combo], ignore_index=True).sort_values("wl")

# 画图（复用前面已有的 draw_bg_mean 与 tight_ylim_from_y）
draw_bg_mean(
    spec_combo,
