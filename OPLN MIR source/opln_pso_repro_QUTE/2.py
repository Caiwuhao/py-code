...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--qpm-period-um",
        type=float,
        default=_default(build_geometry, "qpm_period_um"),
        help="explicit period; omit for Sellmeier (QUTE Table B1: 15.497)",
    )
    parser.add_argument(
        "--length-rounding",
        choices=["floor", "nearest"],
        default=_default(build_geometry, "length_rounding"),
    )
...
def main() -> None:
    ...
    geometry = build_geometry(
        ...
        qpm_period_um=args.qpm_period_um,
        length_rounding=args.length_rounding,
    )
